...
def app():
    ...
    if dataset == "Clubs":
        ...
        #Adding competition names
        df_appearances = pd.merge(df_appearances,df_competitions[['competition_id','name']], left_on = 'competition_id', right_on= 'competition_id')

        #Filtering only League competitions
        df_appearances = pd.merge(df_appearances,df_leagues[['league_id']],how='inner', left_on= 'competition_id' , right_on='league_id')

        #Adding home club names
        df_appearances = pd.merge(df_appearances,df_clubs[['club_id','pretty_name']], left_on='player_club_id', right_on ='club_id')

        #Changing pretty names column name
        df_appearances["Club_name"] = df_appearances['pretty_name']

        #Adding home club names
        df_appearances = pd.merge(df_appearances,df_players[['player_id','pretty_name','country_of_birth','position']], left_on='player_id', right_on ='player_id')

        #Changing pretty names column name
        df_appearances["Player_name"] = df_appearances['pretty_name_y']

        #Adding season to df
        df_appearances = pd.merge(df_appearances,df_games[['game_id','season']], left_on='game_id', right_on ='game_id')
        ...
        df_appearances = df_appearances.query("season >= @start_year").query("season <= @last_year").query("name == @competition_choice")
        ...
        fig_goals = px.bar(df_goals.sort_values('goals',ascending=False).head(25), x='Player_name', y='goals', title = "Top 25 goal scorers", color='goals')
        fig_assists = px.bar(df_goals.sort_values('assists',ascending=False).head(25), x='Player_name', y='assists', title = "Top 25 assists leaders", color="assists")
        fig_g_a= px.bar(df_goals.sort_values('G+A',ascending=False).head(25), x='Player_name', y='G+A', title = "Top 25 assists leaders", color = "G+A", hover_data=["goals","assists"])
